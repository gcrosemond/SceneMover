...
def apply_optional_blocks(template, token_values):
    """
    Optional group syntax: [[ ... ]].
    A group is kept only if at least one token inside has a non-empty value.
    Works with both {token} and {_token} references.
    """
    group_re = re.compile(r"\[\[(.*?)\]\]")
    token_re = re.compile(r"\{(_?[A-Za-z0-9-]+)\}")
    result = template

    while True:
        m = group_re.search(result)
        if not m:
            break
        block = m.group(1)
        refs = token_re.findall(block)
        keep = any(token_values.get(t[1:] if t.startswith("_") else t, "") for t in refs)
        replacement = block if keep else ""
...
def render_template(template, scene, file_rec, config):
    ...
    template = apply_optional_blocks(template, tokens)

    for key, val in tokens.items():
        if ("{_" + key + "}") in template and not val:
            warn(f"  Optional token '{{_{key}}}' is empty for scene '{scene.get('title') or scene.get('id')}'")
    ...
    for key, val in tokens.items():
        ...
        # Optional token: {_key}
        result = result.replace("{_" + key + "}", val)
...
def build_destination(root, scene, file_rec, config):
    ...
    parts = [p for p in rendered.replace("\\", "/").split("/") if p]
...
def validate_scene(scene, root):
    ...
    # Tokens inside [[ ... ]] are conditional and therefore optional for validation.
    template_for_required = re.sub(r"\[\[(.*?)\]\]", "", template)
    # Required token syntax: {token}
    # Optional token syntax: {_token}
    used_tokens = re.findall(r"\{(_?[A-Za-z0-9-]+)\}", template_for_required)
    missing = []
    for token in used_tokens:
        optional = token.startswith("_")
        key = token[1:] if optional else token
        if optional:
            continue
        if key in token_values and not token_values[key]:
            missing.append("{" + key + "}")
    return f"Missing: {', '.join(sorted(set(missing)))}" if missing else None
...
def clean_rendered(s):
    ...
    s = re.sub(r' {2,}', ' ', s)
    # Keep Windows/Unix absolute prefixes intact while cleaning components.
    sep_out = '\\' if '\\' in s else '/'
    s_norm = s.replace('\\', '/')
    ...
    elif len(s_norm) >= 2 and s_norm[1] == ":":
        prefix = s_norm[:2]
        body = s_norm[2:]
        if body.startswith("/"):
            prefix += "/"
...
def fit_to_max_path(dest_folder, dest_basename, scene, file_rec, root, config):
    ...
    def try_build(title_val, perfs_val):
        ...
        result = apply_optional_blocks(template, tokens)
        for key, val in tokens.items():
            # Required token: {key}
            result = result.replace("{" + key + "}", val)
            # Optional token: {_key}
